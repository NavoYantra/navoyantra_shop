...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const products: Product[] = React.useMemo(() => {
    return rawProducts.map((p: any) => ({
      ...
      images: p.images || [],
      specs: p.dimensions?.technicalSpecs?.reduce((acc: any, curr: any) => { if(curr.key) acc[curr.key] = curr.value; return acc; }, {}) || {},
      whatsInside: p.dimensions?.whatsInside?.map((x: any) => x.value) || [],
      sampleProjects: p.dimensions?.sampleProjects?.map((x: any) => x.value) || [],
      inStock: p.stock > 0,
