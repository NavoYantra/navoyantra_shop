...
import { getBlogCategories, getBlogTags } from '../../lib/api';
...
export const AdminBlogs: React.FC = () => {
  ...
  const { data: dbCategories = [] } = useQuery({ queryKey: ['blog_categories'], queryFn: getBlogCategories });
  const { data: dbTags = [] } = useQuery({ queryKey: ['blog_tags'], queryFn: getBlogTags });
  ...
  const handleEdit = (blog: any) => {
    ...
    setCategories(blog.categories || (blog.category ? [blog.category] : []));
                  ...
                  <div className="flex flex-wrap gap-2">
                    {dbCategories.map((cat: any) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${categories.includes(cat.name) ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                      >
                        {cat.name}
                      </button>
                    ))}
                    {dbCategories.length === 0 && <span className="text-xs text-slate-400">No categories defined. Go to Categories to add some.</span>}
                  </div>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {dbTags.map((tag: any) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${tags.includes(tag.name) ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                      >
                        #{tag.name}
                      </button>
                    ))}
                    {dbTags.length === 0 && <span className="text-xs text-slate-400">No tags defined. Go to Tags to add some.</span>}
                  </div>
                </div>
