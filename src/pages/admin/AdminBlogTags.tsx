...
import { getBlogTags, createBlogTag, deleteBlogTag } from '../../lib/api';
...
export const AdminBlogTags: React.FC = () => {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      showToast(error.message || 'Failed to create tag', 'warning');
    }
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      showToast(error.message || 'Failed to delete tag', 'warning');
    }
  });
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Blog Tags</h1>
            <p className="text-sm text-slate-500">Manage predefined tags for blog posts</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1">
          <form onSubmit={handleCreate} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
            <h2 className="text-lg font-bold text-slate-900 mb-4">Add New Tag</h2>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {isLoading ? (
                    <tr>
                      <td colSpan={3} className="p-8 text-center text-slate-500">Loading tags...</td>
                    </tr>
                  ) : tags.length === 0 ? (
                    <tr>
                      <td colSpan={3} className="p-8 text-center text-slate-500">No tags found.</td>
                    </tr>
                  ) : (
                    tags.map((tag: any) => (
                      <tr key={tag.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="p-4 font-bold text-slate-900">#{tag.name}</td>
                        <td className="p-4 text-slate-500 text-sm font-mono">{tag.slug}</td>
