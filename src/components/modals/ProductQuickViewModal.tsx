...
import { 
  X, Star, ShoppingBag, Heart, ShieldCheck, CheckCircle2, Cpu, Box, Sparkles 
} from 'lucide-react';
...
export const ProductQuickViewModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="w-full max-w-4xl bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200 my-8">
        
        {/* Top Header */}
        <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-950 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase">
              {product.category}
            </span>
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 space-y-2">
              <div className="flex items-center space-x-2 text-xs font-bold text-slate-700 dark:text-slate-300">
                ...
                <span>Microcontroller: {product.specs.microcontroller}</span>
              </div>
              <div className="flex items-center space-x-2 text-xs font-bold text-slate-700 dark:text-slate-300">
          ...
          <div className="space-y-6">
            ...
            {/* Pricing Box */}
            <div className="p-4 rounded-2xl bg-blue-50/60 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/50 flex items-center justify-between">
              <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {product.sampleProjects.map((proj, i) => (
                  <span key={i} className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 text-xs font-medium">
                    {proj}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-center space-x-3 pt-2">
              ...
              <button
                ...
                className={`p-4 rounded-2xl border border-slate-200 dark:border-slate-700 transition-colors ${
                  isWishlisted ? 'bg-rose-500 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-rose-500 hover:text-white'
                }`}
              >
